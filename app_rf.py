...
from indoor_nav import load_fingerprints, RandomForestPositioner
...
def build_graph(records, max_d=4.0):
    ...
    EXPLICIT_EDGES = [
        ("A-419 lobby_3", "A-419 lobby"),
        ("A-419 lobby_3", "A - 419 lobby"),
        ("A-419 lobby", "A-420, A-419 lobby"),
        ("A - 419 lobby", "A-420, A-419 lobby"),
        ("A-420, A-419 lobby", "A-420 door1"),
        ("A-419 lobby", "lobby1"),
        ("Elevator 4", "HCD Board (Wing A)"),
    ...
    # Add explicit edges safely
    for e1, e2 in EXPLICIT_EDGES:
        if e1 in nodes and e2 in nodes:
            ek = tuple(sorted([e1, e2]))
            if ek not in added:
                n1_data = nodes[e1]
                n2_data = nodes[e2]
                d = math.sqrt((n2_data["x"] - n1_data["x"])**2 + (n2_data["y"] - n1_data["y"])**2)
                edges.append({"from": e1, "to": e2, "distance": round(d, 2)})
                added.add(ek)
...
def init():
    ...
    # 1. Init Model
    print("[Init] Loading Standard Random Forest")
    raw, db, all_bssids = load_fingerprints("fingerprints_imu.json")
    MODEL = RandomForestPositioner(n_estimators=150)
    MODEL.train(raw, all_bssids)
    
    # Build graph from the raw dataset (so we don't need probabilistic_map.pkl)
...
def api_locate():
    ...
    STATE["filter_info"] = "Standard Random Forest + EMA Smoothing"
